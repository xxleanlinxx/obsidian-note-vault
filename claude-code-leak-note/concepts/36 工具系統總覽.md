...
> [!example] claw-code 最小骨幹 — ToolDefinition
> 來源：`claw-code/src/Tool.py`（~15 行）

![[tool-implementation#^code-core]]

**為什麼這個 stub 能闡明本概念**：
`ToolDefinition` 將工具精煉為最本質的二元組：`name` + `purpose`。這揭示了一個核心設計——工具首先是一個「名稱 + 意圖」的配對，所有更複雜的概念（權限、schema、handler）都建築在這個簡單基礎上。

**本概念的 stub 未呈現的**：
- JSON Schema 參數定義（Zod validation）
- 權限需求宣告（`checkPermissions`）
- 工具執行函式綁定（`call()`）
- 條件啟用與 Feature Flags

→ 完整程式碼走讀 + 差距分析：[[tool-implementation]]

---

> [!example] claw-code 最小骨幹 — Tools Registry
> 來源：`claw-code/src/tools.py`（~90 行）

![[tools-implementation#^code-core]]

**為什麼這個 stub 能闡明本概念**：
`get_tools()` 展示了工具系統的三層篩選：mode → MCP → permission。`execute_tool()` 模擬了「查找 → 執行 → 回傳」的完整流程。讀者能清楚看到工具集合不是靜態的——它根據 simple_mode、MCP 設定、權限動態組裝。
...
→ 完整程式碼走讀 + 差距分析：[[tools-implementation]]
...
- [[tool-implementation]] — ToolDefinition 完整實作筆記
- [[tools-implementation]] — Tools Registry 完整實作筆記
